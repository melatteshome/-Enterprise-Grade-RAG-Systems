...
def TF_IDF():
    ...
    relevant_chunks = sentences[
        : min(2, len(sentences))
    ]  # Extract the first 2 sentences as relevant chunks
    print("Relevant Chunks:", relevant_chunks)

    # Step 4: Embedding Test - Use word embeddings (this is just a placeholder and may require pre-trained embeddings)
    word_embeddings = {"natural": [0.1, 0.2, 0.3], "language": [0.4, 0.5, 0.6]}
    ...
    average_prompt_embedding = (
        [sum(dim) / len(dim) for dim in zip(*prompt_embedding)]
        if prompt_embedding
        else None
    )
...
def _conf(**kwargs):
    ...
    contexts = [
        "Context A is about topic X.",
        "Context B focuses on topic Y.",
        "Context C covers topic Z.",
    ]
    user_questions = [
        "Can you provide information on topic X?",
        "Tell me about topic Y.",
        "What is covered in context C?",
    ]
